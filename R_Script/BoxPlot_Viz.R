#Data frame from the DataMunging script
year.val = 2012
graph.data <- NetExportsData %>% mutate(report.year = year(report.date)) %>% filter(report.year == year.val)   
...
chart.title.first.sentence = paste(zero.and.ok.count," ",ifelse(zero.and.ok.count==1,"country","countries")," had the same imports and exports amount", sep="")
...
p <- p + geom_jitter(alpha = 0.2, height = 0, width = 0.9) 
p <- p + geom_boxplot(alpha = 0.8) 
p <- p + theme(legend.position="none")
p <- p + annotate(geom="text", x=2.5, y=14.25, label=chart.title)
